...
def adaptive_shell_algorithm(kernel_data, queries, k=1, num_spheres=50):
    print(f"Adaptive Shell algorithm start with k={k}, num_spheres={num_spheres}")
...
def compare_num_spheres(kernel_data, queries, num_trials=5):
    num_spheres_values = [10, 30, 50, 70, 90]
    adaptive_results = {}
    
    for num_spheres in num_spheres_values:
        trial_results = []
        for _ in range(num_trials):
            results, overall_error, execution_time = adaptive_shell_algorithm(kernel_data, queries, num_spheres=num_spheres)
            trial_results.append((overall_error, execution_time))
        
        avg_error = np.mean([r[0] for r in trial_results])
        avg_time = np.mean([r[1] for r in trial_results])
        adaptive_results[num_spheres] = (results, avg_error, avg_time)
...
def plot_num_spheres_comparison(adaptive_results, hashing_results):
    num_spheres_values = list(adaptive_results.keys())
    ...
    plt.figure(figsize=(12, 6))
    plt.plot(num_spheres_values, errors, 'bo-', label='Adaptive Shell')
    plt.axhline(y=hashing_error, color='r', linestyle='--', label='Hashing Based')
    plt.xlabel('Number of Spheres')
    plt.ylabel('Average Overall Error')
    plt.title('Error Comparison: Adaptive Shell (varying num_spheres) vs Hashing Based')
    plt.legend()
    plt.show()
    
    plt.figure(figsize=(12, 6))
    plt.plot(num_spheres_values, times, 'bo-', label='Adaptive Shell')
    plt.axhline(y=hashing_time, color='r', linestyle='--', label='Hashing Based')
    plt.xlabel('Number of Spheres')
    plt.ylabel('Average Execution Time (seconds)')
    plt.title('Execution Time Comparison: Adaptive Shell (varying num_spheres) vs Hashing Based')
    plt.legend()
    plt.show()
    
    plt.figure(figsize=(12, 6))
    plt.scatter(times, errors, label='Adaptive Shell')
    for num_spheres, time, error in zip(num_spheres_values, times, errors):
        plt.annotate(f'n={num_spheres}', (time, error))
    plt.scatter([hashing_time], [hashing_error], color='r', label='Hashing Based')
    plt.annotate('Hashing', (hashing_time, hashing_error))
    plt.xlabel('Average Execution Time (seconds)')
    plt.ylabel('Average Overall Error')
    plt.title('Error vs Execution Time: Adaptive Shell (varying num_spheres) vs Hashing Based')
    plt.legend()
    plt.show()
...
print(f"\nHashing Based Algorithm (k-value comparison):")
print(f"Average execution time: {k_hashing_results[2]:.2f} seconds")
print(f"Average overall error: {k_hashing_results[1]:.10f}")

print("\nNumber of spheres comparison results:")
for num_spheres, (results, avg_error, avg_time) in spheres_adaptive_results.items():
    print(f"\nAdaptive Shell Algorithm (num_spheres={num_spheres}):")
    print(f"Average execution time: {avg_time:.2f} seconds")
    print(f"Average overall error: {avg_error:.10f}")

print(f"\nHashing Based Algorithm (num_spheres comparison):")
print(f"Average execution time: {spheres_hashing_results[2]:.2f} seconds")
print(f"Average overall error: {spheres_hashing_results[1]:.10f}")