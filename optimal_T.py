...
def optimal_T(data, fun, query, delta, epsilon):
    T_values = rs.generate_sampling_points(len(data))
    kde_true = rs.kernel_density_true(data, query)
    for T in T_values:
        num_true = 0
        num_false = 0
        for _ in range(1000):
            ...
            if mean_kde >= (1 - epsilon) * kde_true and mean_kde <= (1 + epsilon) * kde_true:
                ...
                num_false += 1
        if(num_false/1000 <= delta):
            return T
    return None

#computing optimal value of T for various combinations of delta and epsilon
...
eval_epsilons = [0.00001, 0.0001, 0.001, 0.01, 0.1, 0.5, 1]
...
for i, eval_delta in enumerate(eval_deltas):
    for j, eval_epsilon in enumerate(eval_epsilons):
        T = optimal_T(rs.kernel_data, rs.gaussian_kernel, rs.query, eval_delta, eval_epsilon)
        optimalTs_gaussian[i, j] = T
        print(f"delta = {eval_delta}, epsilon = {eval_epsilon}, optimal T is {T}")

optimalTs_student = np.zeros((len(eval_delta), len(eval_epsilon)))
for i, eval_delta in enumerate(eval_deltas):
    for j, eval_epsilon in enumerate(eval_epsilons):
        T = optimal_T(rs.kernel_data, rs.student_kernel, rs.query, eval_delta, eval_epsilon)
