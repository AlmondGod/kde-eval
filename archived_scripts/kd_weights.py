...
def kd_basic_gini(data, fun, query): #not the right gini I think
    kde_estimates = kd_weights(data, fun, query)
    weights = np.array(kde_estimates)
    weights = weights / np.sum(weights)
...
def kd_gini(data, fun, query): #this is based on one I found online, 
    #there's many different formulas I've found (most don't work on a dataset this large)
    kde_estimates = kd_weights(data, fun, query)
    weights = np.array(kde_estimates)
    weights = weights / np.sum(weights)
...
#         f.write(f"query_data[{i}], {gini_g}\n")
        
#     with open("outputs/kd_weights_student.txt", 'a') as f:
#         f.write(f"query_data[{i}], {gini_s}\n")
